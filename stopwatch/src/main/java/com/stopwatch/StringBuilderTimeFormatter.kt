...
internal class StringBuilderTimeFormatter(private val semantic: Semantic) : TimeFormatter {
  ...
  override fun format(millis: Long): CharSequence {
    ...
    var millisToShow = Constants.TimeValues.NONE
    var secondsToShow = Constants.TimeValues.NONE
    var minutesToShow = Constants.TimeValues.NONE
    var hoursToShow = Constants.TimeValues.NONE
  ...
  private fun timeUnitsOf(millis: Long): TimeContainer {
    val seconds = millis / Constants.TimeValues.MILLIS_IN_SECOND
    val minutes = seconds / Constants.TimeValues.SECONDS_IN_MINUTE
    val hours = minutes / Constants.TimeValues.MINUTES_IN_HOUR
    val remMillis = millis % Constants.TimeValues.MILLIS_IN_SECOND
    val remSeconds = seconds - minutes * Constants.TimeValues.SECONDS_IN_MINUTE
    val remMinutes = minutes - hours * Constants.TimeValues.MINUTES_IN_HOUR
  ...
  private fun applyFormat(millisToShow: Long, secondsToShow: Long,
    ...
    if (millisToShow != Constants.TimeValues.NONE) {
      updateString(millisToShow, TimeUnitType.R_MILLISECONDS)
    }
    if (secondsToShow != Constants.TimeValues.NONE) {
      updateString(secondsToShow, TimeUnitType.SECONDS)
    }
    if (minutesToShow != Constants.TimeValues.NONE) {
      updateString(minutesToShow, TimeUnitType.MINUTES)
    }
    if (hoursToShow != Constants.TimeValues.NONE) {
