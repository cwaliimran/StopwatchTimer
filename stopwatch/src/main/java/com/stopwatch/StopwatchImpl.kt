...
internal class StopwatchImpl(
  ...
  private var initialTime: Long = Constants.TimeValues.NONE
  ...
  override fun start() {
    if (state == TimeCountingState.RESUMED) return
    if (initialTime == Constants.TimeValues.NONE) {
  ...
  override fun reset() {
    ...
    initialTime = Constants.TimeValues.NONE
