...
import com.stopwatch.Constants.Symbols.isOneOfSpecialSymbols
...
internal object AnalyzerImpl : Analyzer {
    ...
    override fun analyze(format: String): SemanticImpl {
        ...
        return SemanticImpl(
            hours, minutes, seconds, rMillis, format, strippedFormat, smallestUnit, largestUnit
        )
    }
    ...
    private fun getPositionOf(timeUnitType: TimeUnitType, input: String): Position {
        ...
        for (i in input.indices) {
            val symbol = input[i]
            if (isSymbolNotEscapedAndEqualTo(
                    timeUnitType,
                    input,
                    i
                ) && start != -1 && i - 2 > 0 && !isSymbolNotEscapedAndEqualTo(
                    timeUnitType, input, i - 1
                )
            ) {
                throw NonContiguousFormatSymbolsException(
                    "Time unit " + timeUnitType.value + " was found several times in the format"
                )
            }
            if (symbol == timeUnitChar) {
                if (i == 0) {
                    ...
                    start = end
                } else if (input[i - 1] != Constants.Symbols.SYMBOL_ESCAPE) {
                    if (start == -1) {
                        start = i
                    }
                    end = i
    ...
    private fun isSymbolNotEscapedAndEqualTo(
        timeUnitType: TimeUnitType, input: String, position: Int
    ): Boolean {
        if (position - 1 < 0) {
            return false
        }
        ...
        return prev != Constants.Symbols.SYMBOL_ESCAPE && symbol == timeUnitType.value
    }

    private fun validatePositions(
        hours: Position, minutes: Position, seconds: Position, rMillis: Position
    ) {
        if (hours.isEmpty && minutes.isEmpty && seconds.isEmpty && rMillis.isEmpty) {
            throw NoNecessarySymbolsException(
                "No special symbols like " + Constants.Symbols.SYMBOL_HOURS + ", " + Constants.Symbols.SYMBOL_MINUTES + ",  " + Constants.Symbols.SYMBOL_SECONDS + " or " + Constants.Symbols.SYMBOL_REM_MILLIS + "was found in the format"
            )
        }
    }
    ...
    private fun getSmallestAvailableUnit(
        minutes: Position, seconds: Position, rMillis: Position
    ): TimeUnitType {
    ...
    private fun stripFormat(format: String): String {
        ...
        for (i in format.indices) {
            val symbol = format[i]
            if (symbol == Constants.Symbols.SYMBOL_ESCAPE && i < format.length - 1) {
                val next = format[i + 1]
    ...
    private fun numberOfEscapeSymbolsBefore(input: String, position: Int): Int {
        ...
        for (i in 0 until position - 1) {
            val symbol = input[i]
            val next = input[i + 1]
            if (symbol == Constants.Symbols.SYMBOL_ESCAPE && next.isOneOfSpecialSymbols()) {
                count++
            }
        }
        return count
